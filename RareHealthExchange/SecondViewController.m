...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.mode == 1) {
        ...
        if (self.currentSymptomIndex<[self.symptomArray count]-1) {
            ...
            self.PromptLabel.text = [NSString stringWithFormat:@"These are the severity levels for %@", symptomName];
...
- (IBAction)nextPress:(id)sender {
    ...
    if (self.currentSymptomIndex<[self.symptomArray count]-1) {
        ...
        self.PromptLabel.text = [NSString stringWithFormat:@"These are the severity levels for the symptom %@", symptomName];
        [self.testArray addObjectsFromArray:[self.symptomSeverityDescriptionAndID keysSortedByValueUsingSelector:@selector(localizedStandardCompare:)]];
        [self.ResultsTable reloadData];
...
-(void)startFresh{
    ...
    self.jsonSymptomData = [NSArray new];
    self.SearchText.text = @"";
    self.PromptLabel.text = @"Select a disease to learn about...";
...
- (void)postDiseaseID{
    ...
    else
    {
        ...
        if (self.jsonSymptomData.count > 0) {
            ...
            self.PromptLabel.text = [NSString stringWithFormat:@"These are severity levels for %@", symptomName];
